...
void loadLevel(String level){
  ...
  Blip newBlip = new Blip(goodPlayer.posX, arenaHeight, antiSkyColour);
...
void loadReplay(String replay){
  ...
  String[] tempReplayStrings = split(replayStrings[0],";");
  replayStrings = new String[tempReplayStrings.length];
  for(int i = 0; i < tempReplayStrings.length; i++){
     replayStrings[i] = tempReplayStrings[i];
  ...
  for(int i = 0; i < replayStrings.length; i++){
    String[] tempString = split(replayStrings[i], ",");
    println(tempString[0]);
    Blip newBlip = new Blip(float(tempString[0]), 0, antiSkyColour);
    blipList.add(newBlip);
    boostList.add(float(tempString[1]));
  }
  

  //Step 4 - plonk player at starting coordinates (paused and in autopilot mode)
